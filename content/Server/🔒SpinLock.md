...
Lock 구현에는 여러 구현방식이있는데
그중에 SpinLock계열을 설명하려고합니다
...
| 방식           | 설명                        | 장점                      | 단점                |
| ------------ | ------------------------- | ----------------------- | ----------------- |
| **SpinLock** | 잠금이 풀릴 때까지 계속 루프          | 짧은 작업에 효율적, 컨텍스트 스위칭 없음 | 오래 걸리면 CPU 점유율 급증 |
| **Yield**    | 잠금 실패 시 CPU 점유권 반납        | CPU 낭비 줄임               | 컨텍스트 스위칭 오버헤드     |
| **Event**    | OS에 이벤트 등록 후 신호 받을 때까지 대기 | CPU 낭비 거의 없음            | 이벤트 처리 비용, 구현 복잡  |
